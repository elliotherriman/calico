...
//
// #stop: song will stop that song, or #stop will stop all songs
...
{
	property = process(story, property);
	audio.pause(story, property, property.options); 
});

Tags.add("resume", function(story, property)
{
	property = process(story, property);
	audio.resume(story, property, property.options); 
});

Tags.add("stop", function(story, property)
{
	property = process(story, property);
	audio.stop(story, property, property.options); 
});
...
function process(story, property)
{
	...
	if (!property.value || !property.value[0]) return false;

	property.name = property.value[0][0] || "";

	if (property.name)
	{
		property.path = addFileType(property.name, 
		story.options.defaultaudioformat, 
		story.options.defaultaudiolocation);
	}
...
class audio
{
	...
	static pause(story, file, options = {})
	{
		...
		{
			audio.sounds.forEach((track) => track.fade(audio.volume, 0, options.duration));
		}		
	}
	...
	static resume(story, file, options = {})
	{
		...
		{
			audio.sounds.forEach((track) => track.fade(0, audio.volume, options.duration));
		}
	}

	static get volume ()
	{
		if (!audio._volume) audio._volume = 0.5;
	...
	{
		if (!value) return;
		audio.sounds.forEach((track) => track.fade(audio.volume, value))
		audio._volume = value;
	}
}
