(function (global, factory) {
	...
	class AbstractValue extends InkObject {
			static Create(val, preferredNumberType) {
					...
					if (preferredNumberType) {
							if (preferredNumberType === exports.ValueType.Int &&
									Number.isInteger(Number(val))) {
									return new IntValue(Number(val));
							}
							else if (preferredNumberType === exports.ValueType.Float &&
									!isNaN(val)) {
	...
	class IntValue extends Value {
			...
			Cast(newType) {
					...
					if (newType == exports.ValueType.Float) {
							return new FloatValue(this.value);
					}
					if (newType == exports.ValueType.String) {
	...
	class FloatValue extends Value {
			...
			get valueType() {
					return exports.ValueType.Float;
			}
			Cast(newType) {
					if (this.value === null)
							return throwNullException("Value.value");
					...
					if (newType == exports.ValueType.Int) {
	...
	class StringValue extends Value {
			...
			get valueType() {
					return exports.ValueType.String;
			}
			get isTruthy() {
					if (this.value === null)
							return throwNullException("Value.value");
	...
	class DivertTargetValue extends Value {
			...
			get valueType() {
					return exports.ValueType.DivertTarget;
			}
			get targetPath() {
					if (this.value === null)
							return throwNullException("Value.value");
	...
	class VariablePointerValue extends Value {
			...
			get valueType() {
					return exports.ValueType.VariablePointer;
			}
			get isTruthy() {
	...
	class ListValue extends Value {
			...
			Cast(newType) {
					...
					else if (newType == exports.ValueType.Float) {
							let max = this.value.maxItem;
							if (max.Key.isNull)
					...
					else if (newType == exports.ValueType.String) {
							let max = this.value.maxItem;
							if (max.Key.isNull)
	...
	})(exports.PushPopType || (exports.PushPopType = {}));
			...
			toString() {
					if (this.hasVariableTarget) {
							...
							if (this.pushesToStack) {
									if (this.stackPushType == exports.PushPopType.Function) {
											sb.Append(" function");
									}
									else {
											sb.Append(" tunnel");
	...
	class NativeFunctionCall extends InkObject {
			...
			Call(parameters) {
					...
					if (coercedType == exports.ValueType.Int) {
							return this.CallType(coercedParams);
					}
					else if (coercedType == exports.ValueType.Float) {
							return this.CallType(coercedParams);
					}
					else if (coercedType == exports.ValueType.String) {
							return this.CallType(coercedParams);
					}
					else if (coercedType == exports.ValueType.DivertTarget) {
							return this.CallType(coercedParams);
					}
					else if (coercedType == exports.ValueType.List) {
							return this.CallType(coercedParams);
					}
					return null;
			}
			CallType(parametersOfSingleType) {
					...
					if (paramCount == 2 || paramCount == 1) {
							...
							if (!opForTypeObj) {
									const key = exports.ValueType[valType];
							...
							if (paramCount == 2) {
									...
									if (this.name === NativeFunctionCall.Int) {
											return Value.Create(resultVal, exports.ValueType.Int);
									}
									else if (this.name === NativeFunctionCall.Float) {
											return Value.Create(resultVal, exports.ValueType.Float);
									}
									else {
			...
			CallBinaryListOperation(parameters) {
					...
					if ((this.name == "&&" || this.name == "||") &&
							(v1.valueType != exports.ValueType.List || v2.valueType != exports.ValueType.List)) {
							if (this._operationFuncs === null)
									return throwNullException("NativeFunctionCall._operationFuncs");
							let op = this._operationFuncs.get(exports.ValueType.Int);
							if (op === null)
					...
					if (v1.valueType == exports.ValueType.List && v2.valueType == exports.ValueType.List)
							return this.CallType([v1, v2]);
					throw new StoryException("Can not call use " +
							this.name +
							" operation on " +
							exports.ValueType[v1.valueType] +
							" and " +
							exports.ValueType[v2.valueType]);
			}
			CallListIncrementOperation(listIntParams) {
					...
					for (let [listItemKey, listItemValue] of listVal.value) {
							...
							if (this._operationFuncs === null)
									return throwNullException("NativeFunctionCall._operationFuncs");
							let intOp = this._operationFuncs.get(exports.ValueType.Int);
			...
			CoerceValuesToSingleType(parametersIn) {
					let valType = exports.ValueType.Int;
					...
					for (let obj of parametersIn) {
							...
							if (val.valueType == exports.ValueType.List) {
									specialCaseList = asOrNull(val, ListValue);
							}
					}
					let parametersOut = [];
					if (exports.ValueType[valType] == exports.ValueType[exports.ValueType.List]) {
							for (let inkObjectVal of parametersIn) {
									let val = asOrThrows(inkObjectVal, Value);
									if (val.valueType == exports.ValueType.List) {
											parametersOut.push(val);
									}
									else if (val.valueType == exports.ValueType.Int) {
											...
											const key = exports.ValueType[val.valueType];
			...
			static GenerateNativeFunctionsIfNecessary() {
					if (this._nativeFunctions == null) {
							...
							this.AddOpToNativeFunc(this.Equal, 2, exports.ValueType.DivertTarget, divertTargetsEqual);
							this.AddOpToNativeFunc(this.NotEquals, 2, exports.ValueType.DivertTarget, divertTargetsNotEqual);
					}
			}
			...
			static AddIntBinaryOp(name, op) {
					this.AddOpToNativeFunc(name, 2, exports.ValueType.Int, op);
			}
			static AddIntUnaryOp(name, op) {
					this.AddOpToNativeFunc(name, 1, exports.ValueType.Int, op);
			}
			static AddFloatBinaryOp(name, op) {
					this.AddOpToNativeFunc(name, 2, exports.ValueType.Float, op);
			}
			static AddFloatUnaryOp(name, op) {
					this.AddOpToNativeFunc(name, 1, exports.ValueType.Float, op);
			}
			static AddStringBinaryOp(name, op) {
					this.AddOpToNativeFunc(name, 2, exports.ValueType.String, op);
			}
			static AddListBinaryOp(name, op) {
					this.AddOpToNativeFunc(name, 2, exports.ValueType.List, op);
			}
			static AddListUnaryOp(name, op) {
					this.AddOpToNativeFunc(name, 1, exports.ValueType.List, op);
			}
			toString() {
	...
	class JsonSerialisation {
			...
			static WriteRuntimeObject(writer, obj) {
					...
					if (divert) {
							...
							if (divert.isExternal) {
									...
									if (divert.stackPushType == exports.PushPopType.Function) {
											divTypeKey = "f()";
									}
									else if (divert.stackPushType == exports.PushPopType.Tunnel) {
											divTypeKey = "->t->";
									}
							}
							let targetStr;
			...
			static JTokenToRuntimeObject(token) {
					...
					if (typeof token === "object" && !Array.isArray(token)) {
							...
							let divPushType = exports.PushPopType.Function;
							let external = false;
							if ((propValue = obj["->"])) {
									isDivert = true;
							}
									...
									isDivert = true;
									pushesToStack = true;
									divPushType = exports.PushPopType.Function;
							}
							else if ((propValue = obj["->t->"])) {
									isDivert = true;
									pushesToStack = true;
									divPushType = exports.PushPopType.Tunnel;
							}
					...
					if (token.constructor == Object && Object.keys(token).length == 1) {
							var str = this.JTokenToRuntimeObject(Object.keys(token)[0]);
							if (str && asOrNull(str, StringValue)) {
									str.debugMetadata = token[Object.keys(token)[0]];
									return str;
							}
					}
	...
	class Story extends InkObject {
					...
					if (outputStreamEndsInNewline || !this.canContinue) {
							...
							if (!this.canContinue) {
									...
									if (this.state.generatedChoices.length == 0 &&
											...
											this._temporaryEvaluationContainer == null) {
											if (this.state.callStack.CanPop(exports.PushPopType.Tunnel))
													this.AddError("unexpectedly reached end of content. Do you need a '->->' to return from a tunnel?");
											else if (this.state.callStack.CanPop(exports.PushPopType.Function))
							...
							switch (evalCommand.commandType) {
									...
									case ControlCommand.CommandType.PopTunnel:
											let popType = evalCommand.commandType == ControlCommand.CommandType.PopFunction
													? exports.PushPopType.Function
													: exports.PushPopType.Tunnel;
											let overrideTunnelReturnTarget = null;
											if (popType == exports.PushPopType.Tunnel) {
											...
											if (this.state.TryExitFunctionEvaluationFromGame()) {
													...
													let names = new Map();
													names.set(exports.PushPopType.Function, "function return statement (~ return)");
													names.set(exports.PushPopType.Tunnel, "tunnel onwards statement (->->)");
			...
			ChoosePathString(path, resetCallstack = true, args = []) {
					...
					if (resetCallstack) {
							...
							if (this.state.callStack.currentElement.type == exports.PushPopType.Function) {
									let funcDetail = "";
			...
			EvaluateExpression(exprContainer) {
					...
					this.state.callStack.Push(exports.PushPopType.Tunnel);
			...
			CallExternalFunction(funcName, numberOfArguments) {
					...
					if (!foundExternal) {
							if (this.allowExternalFunctionFallbacks) {
									...
									this.state.callStack.Push(exports.PushPopType.Function, undefined, this.state.outputStream.length);
			...
			ObserveVariable(variableName, observer) {
					...
					if (!this.state.variablesState.GlobalVariableExistsWithName(variableName))
							throw new StoryException("Cannot observe variable '" +
									variableName +
			...
			NextContent() {
					...
					if (!successfulPointerIncrement) {
							let didPop = false;
							if (this.state.callStack.CanPop(exports.PushPopType.Function)) {
									this.state.PopCallStack(exports.PushPopType.Function);
									if (this.state.inExpressionEvaluation) {
											this.state.PushEvaluationStack(new Void());
									}
									didPop = true;
	...
	class CallStack {
			...
			Reset() {
					...
					this._threads[0].callstack.push(new CallStack.Element(exports.PushPopType.Tunnel, this._startOfRoot));
			}
			...
			get elementIsEvaluateFromGame() {
					return this.currentElement.type == exports.PushPopType.FunctionEvaluationFromGame;
			}
			...
			get callStackTrace() {
					...
					for (let t = 0; t < this._threads.length; t++) {
							...
							for (let i = 0; i < thread.callstack.length; i++) {
									if (thread.callstack[i].type == exports.PushPopType.Function)
											sb.Append("  [FUNCTION] ");
									else
											sb.Append("  [TUNNEL] ");
	...
	class StoryState {
			...
			LoadJsonObj(value) {
					...
					if (jSaveVersion == null) {
							...
							throw new StoryException("Ink save format isn't compatible with the current version (saw '" +
									jSaveVersion +
									"', but minimum is " +
			...
			PushToOutputStreamIndividual(obj) {
					...
					if (glue) {
							...
							if (currEl.type == exports.PushPopType.Function) {
									...
									if (text.isNewline) {
											...
											if (functionTrimIndex > -1) {
													...
													for (let i = callStackElements.length - 1; i >= 0; i--) {
															...
															if (el.type == exports.PushPopType.Function) {
			...
			TrimWhitespaceFromFunctionEnd() {
					Debug.Assert(this.callStack.currentElement.type == exports.PushPopType.Function);
			...
			PopCallStack(popType = null) {
					if (this.callStack.currentElement.type == exports.PushPopType.Function)
			...
			StartFunctionEvaluationFromGame(funcContainer, args) {
					this.callStack.Push(exports.PushPopType.FunctionEvaluationFromGame, this.evaluationStack.length);
			...
			TryExitFunctionEvaluationFromGame() {
					if (this.callStack.currentElement.type ==
							exports.PushPopType.FunctionEvaluationFromGame) {
							this.currentPointer = Pointer.Null;
							this.didSafeExit = true;
							return true;
					}
			...
			CompleteFunctionEvaluationFromGame() {
					...
					this.PopCallStack(exports.PushPopType.FunctionEvaluationFromGame);
					if (returnedObj) {
							...
							if (returnVal.valueType == exports.ValueType.DivertTarget) {
	...
	function inject(target, search, replacement)
	{
